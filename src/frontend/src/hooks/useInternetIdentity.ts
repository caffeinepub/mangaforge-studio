...
export function InternetIdentityProvider({
  ...
  // Store createOptions in a ref so effects can read it without being in their dep arrays
  const createOptionsRef = useRef(createOptions);
  createOptionsRef.current = createOptions;

  // Track whether we've already initialized so the effect only runs once on mount
  ...
  const clear = useCallback(() => {
    ...
    void authClient
      ...
      .then(() => {
        ...
        // Reset initializedRef so the re-init effect can run after logout
  ...
  // Re-initialize after logout (when authClient is cleared back to undefined)
  useEffect(() => {
