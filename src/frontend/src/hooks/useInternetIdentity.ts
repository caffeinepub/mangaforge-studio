...
export function InternetIdentityProvider({
  ...
  // Capture createOptions in a ref so the init effect can use it without re-running
  ...
  // Run initialization exactly ONCE on mount. Never re-run when authClient changes.
  useEffect(() => {
    ...
    // biome-ignore lint/correctness/useExhaustiveDependencies: init runs exactly once on mount
  }, []); // Empty deps: initialize exactly once on mount
